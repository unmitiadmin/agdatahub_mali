...
class AgricultureIndex {
    ...
    init = () => {
      ...
      if (this.oaMap != undefined || this.oaMap != null) {
        this.oaMap.remove();
        this.oaMap.off();
      }
      ...
      $.ajax({
        ...
        success: (response) => {
          this.mali_fao_data = response;
          this.years = [...new Set(response.map((item) => item["year"]))];
          this.crops = [...new Set(response.map((item) => item["crop"]))];
          // Common filter values
          let yearHtml = "<option value='all'>All years</option>";
          yearHtml += this.years
            .map((a) => `<option value="${a}">${a}</option>`)
            .join("\n");
          let cropHtml = "<option value='all'>All crops</option>";
          cropHtml += this.crops
            .map((a) => `<option value="${a}">${a}</option>`)
            .join("\n");
          let indicatorHtml = "<option value='all'>All indicators</option>";
          indicatorHtml += this.indicators
            .map(
              (a) => `<option value="${a.indicator}">${a.indicator}</option>`
            )
            .join("\n");
          // fao filters
          ...
          this.filterFAOYear
            .on("change", (e) => (this.chosenYear = $(e.currentTarget).val()))
            .trigger("change");
          this.filterFAOIndicator
            .on(
              "change",
              (e) => (this.chosenIndicator = $(e.currentTarget).val())
            )
            .trigger("change");
          // oa filters
          this.filterOACrop.empty().html(cropHtml);
        },
      });
    };
    ...
    readDataSets = () => {
      Promise.all([
        ...
        .then(([agricultureData, geoData]) => {
          ...
          [this.oaArea, this.oaYield, this.oaProduction] = this.excelToArray(
            agricultureData,
            "openafrica_area",
            "openafrica_yield",
            "openafrica_production"
          );
          this.geoData = geoData;
          this.plotRegions();
        })
        .then(() => {
          this.generate_yield_data();
          // this.enableFAOApply();
          this.enableOAApply();
        })
        .then(() => {
          this.filterRegion.on("change", (e) => {
            ...
            if (this.regionId) {
              d3.select(`path#oa_region_${this.regionId}`).attr(
                "fill-opacity",
                0.9
              );
              d3.selectAll(`path:not(#oa_region_${this.regionId})`).attr(
                "fill-opacity",
                0.5
              );
    ...
    enableFAOApply = () => {
      this.submitFAO
        ...
        .on("click", () => {
          let indicatorData =
            this.chosenIndicator == "Area"
              ? this.faoArea
              : this.chosenIndicator == "Production"
              ? this.faoProduction
              : this.faoYield;
          let yearData = indicatorData.filter((b) => b.year == this.chosenYear);
          let regionData = this.regionId
            ? yearData.filter((b) => b.region_id == this.regionId)
            : yearData;
          let chartData = this.crops.map((b) => {
            let uqRegionsCount =
              this.uqArray(regionData.map((c) => c.region_id)).length || 1;
            let value =
              this.chosenIndicator == "Yield"
                ? this.sumArray(regionData.map((c) => c[b])) / uqRegionsCount
                : this.sumArray(regionData.map((c) => c[b]));
            return { name: b, value: value };
          });
          let units = this.indicators.find(
            (b) => b.indicator == this.chosenIndicator
          ).units;
          this.plotFAOIndicator(
            chartData,
            units,
    ...
    generate_yield_data = () => {
      this.submitFAO
        .unbind("click")
        .on("click", () => {
          let filtered_fao_data = this.mali_fao_data;
          if (this.chosenYear != "all") {
            filtered_fao_data = this.mali_fao_data.filter(
              (item) => item["year"] == this.chosenYear
            );
          }

          let yield_series = [];
          if (this.chosenIndicator == "all") {
            const summedData = {};
            filtered_fao_data.forEach((item) => {
              const cropName = item["crop"];
              if (summedData[cropName]) {
                summedData[cropName] += Math.round(
                  parseInt(item["yield"]) + parseInt(item["area"]) + parseInt(item["production"])
                );
              } else {
                summedData[cropName] = Math.round(
                    parseInt(item["yield"]) + parseInt(item["area"]) + parseInt(item["production"])
            ...
            filtered_fao_data.forEach((item) => {
              ...
              if (summedData[cropName]) {
                summedData[cropName] += Math.round(
                    parseInt(item[this.chosenIndicator])
                );
              } else {
                summedData[cropName] = Math.round(
                    parseInt(item[this.chosenIndicator])
            ...
            yield_series = [
              {
                name: this.chosenIndicator,
                data: Object.values(summedData),
                color: "#ffe59a",
              },
            ];
          }
          this.generate_fao_bar_graph(yield_series);
        })
        .trigger("click");
    };

    generate_fao_bar_graph = (yield_series) => { 
    ...
    plotFAOIndicator = (chartData, units, indicator, year) => {
      if (chartData.length) {
        ...
        $(`div#${this.chartFAOId}`)
          .empty()
          .html(`<div class="my-5 text-center">Data unavailable</div>`);
      }
    };
      ...
      this.filterOACrop
        ...
        .on("change", (e) => {
          ...
          let chartAreaData = this.regionId
            ? areaData
                .filter((b) => b.region_id == this.regionId)
                .map((b) => {
                  return { year: b.year, value: b.value };
                })
            : areaUqYears.map((y) => {
                let yearTotal = this.sumArray(
                  areaData.filter((b) => b.year == y).map((b) => b.value)
                );
          ...
          let productionUqYears = this.uqArray(
            productionData.map((b) => b.year)
          );
          let chartProductionData = this.regionId
            ...
            : productionUqYears.map((y) => {
                let yearTotal = this.sumArray(
                  productionData.filter((b) => b.year == y).map((b) => b.value)
                );
                return { year: y, value: yearTotal };
              });
          this.plotOAIndicators(
            chartProductionData,
            "chart-oa-production",
            "Production",
            "Ton"
          ...
          let chartYieldData = this.regionId
            ...
            : yieldUqYears.map((y) => {
                let yearTotal = this.sumArray(
                  yieldData.filter((b) => b.year == y).map((b) => b.value)
                );
                return { year: y, value: yearTotal };
              });
          this.plotOAIndicators(
      ...
      let polygons = L.d3SvgOverlay(
        (selection, projection) => {
          ...
          locationGroup
            ...
            .on("mouseenter", (e, d) => {
              d3.select(`path#oa_region_${d.properties.region_id}`).attr(
                "cursor",
                "pointer"
              );
            ...
            .on("click", (e, d) => {
              ...
              d3.selectAll(
                `path:not(#oa_region_${d.properties.region_id})`
              ).attr("fill-opacity", 0.5);
            });
          locationGroup
            .transition()
            .duration(10)
            .attr("stroke-width", `${0.5 / pScale}px`);
        },
        { id: `oa_regions` }
      );
    ...
    reloadSvgHolder = () => {
      this.oaMap.eachLayer((l) => {
        if (l.options.id && l.options.id.indexOf("regions") === 0)
          this.oaMap.removeLayer(l);
      });
    };
    ...
    loadExcelFile = (fileName) =>
      new Promise((resolve, reject) => {
        startWaiting();
        fetch(`./assets/datasets/${fileName}`)
          .then((res) => res.blob())
          .then((file) => {
    ...
    excelToArray = (file, ...sheets) => {
      ...
      return sheets.map((sheet) => {
        ...
        let data = body.map((a) => {
          ...
          header.forEach(
            (b, i) =>
              (result[b] = ["NULL", undefined, null].includes(a[i])
                ? null
                : isNaN(a[i])
                ? a[i].replace(/\"/g, "")
                : this.num(a[i]))
